...
from syrax_search.shortlist import Shortlists
...
def test_a_close_call_offers_a_value_per_candidate_and_one_for_none_of_them():
    ...
    offer = shortlists.offer(close_call())
...
def test_every_tappable_value_fits_what_a_tap_can_carry():
    """Telegram's `callback_data` is 1-64 bytes, and a value over it cannot be put on a button."""
    offer = Shortlists().offer(close_call())
    for value in [*(one["choice"] for one in offer["results"]), offer["none_of_these"]]:
        assert 1 <= len(value.encode()) <= 64
...
def test_a_confident_verdict_offers_nothing_to_choose_between():
    ...
    reply = Shortlists().offer(confident)
...
def test_an_empty_verdict_offers_nothing_at_all():
    reply = Shortlists().offer(Verdict("empty", (), -0.05))
...
def test_a_tap_past_the_end_of_its_own_shortlist_sends_nothing():
    shortlists = Shortlists()
    token = shortlists.offer(close_call())["results"][0]["choice"].rpartition(":")[0]
