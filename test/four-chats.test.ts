...
describe("the four chats", { skip: !runtimeIsInstalled() }, () => {
  ...
  for (const subject of everyChat) {
    it(`answers the ${subject.carrierName} carrier as the ${subject.id} agent`, async () => {
      const { sent, prompt } = await ask(
        "What is the state of things?",
        syrax.carriers[subject.id],
      );

      assert.equal(answeringAgent(prompt), subject.id);
      assert.equal(sent.body.message_thread_id, syrax.carriers[subject.id]);
  ...
  it("redirects a question a chat does not own, rather than reaching across to answer it", async () => {
    const intoMedia = (call: OutboundCall) => call.body.message_thread_id === syrax.carriers.media;
    const wroteToMedia = syrax.telegram.matching("sendMessage", intoMedia).length;

    const { sent, prompt } = await ask("Download the new Dune film.", syrax.carriers.academic);
    ...
    assert.equal(sent.body.message_thread_id, syrax.carriers.academic);
    assert.equal(
      syrax.telegram.matching("sendMessage", intoMedia).length,
      wroteToMedia,
